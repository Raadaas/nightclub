...
export class AdminSettingsComponent implements OnInit {
  ...
  private loadProfile(): void {
    ...
    this.api.get().subscribe({
      ...
      error: () => {
        this.profileError = 'Nije moguće učitati profil.';
        this.isLoadingProfile = false;
      },
    });
  }
  ...
  getRoleLabel(): string {
    ...
    if (this.profile.isAdmin)    return 'Administrator';
    if (this.profile.isManager)  return 'Manager';
    if (this.profile.isEmployee) return 'Zaposleni';
    return 'Korisnik';
  }
  ...
  onSubmitProfile(): void {
    ...
    this.api.update({
      ...
      next: () => {
        ...
        this.toaster.success('Profil uspješno ažuriran');
        this.loadProfile();
      },
      error: (err: HttpErrorResponse) => {
        this.isSubmittingProfile = false;
        this.profileFormError = err?.error?.message ?? 'Greška pri ažuriranju profila.';
      },
    });
  }
  ...
  onSubmitPassword(): void {
    ...
    this.api.changePassword({
      ...
      next: () => {
        ...
        this.toaster.success('Lozinka uspješno promijenjena');
        this.passwordForm.reset();
      },
      error: (err: HttpErrorResponse) => {
        this.isSubmittingPassword = false;
        this.passwordFormError = err?.error?.message ?? 'Greška pri promjeni lozinke.';
      },
    });
  }
